...
Production mode prefers PostgreSQL and live adapters. Demo values are kept only
as an explicit fallback and are labelled in API responses; they are never
presented as official or real-time data.
"""
...
try:
    ...
    psycopg = None
from core.dca_backtest import demo as dca_demo
from core.dca_backtest import run as run_dca
from core.live_data import fetch_fund_valuations

TZ=ZoneInfo("Asia/Shanghai")
app=FastAPI(title="Fund Watch API",version="0.5.0")
...
def save_estimate_snapshot(payload:dict[str,Any])->None:
    ...
    try:
        with psycopg.connect(url,connect_timeout=4) as conn:
            with conn.cursor() as cur:
                cur.execute("INSERT INTO fund_estimate_snapshot(fund_code,observed_at,estimated_nav,change_pct,source,confidence) VALUES(%s,%s,%s,%s,%s,%s)",(payload.get("fund_code"),now_cn(),payload.get("estimated_nav"),payload.get("estimated_change_pct"),payload.get("source"),payload.get("confidence")))
            conn.commit()
...
@app.get("/api")
def root():return {"name":"Fund Watch","version":"0.5.0","status":"ok","mode":"postgres" if os.getenv("DATABASE_URL") else "demo"}
@app.get("/api/health")
def health():
    return {"code":0,"status":"healthy","mode":"postgres" if os.getenv("DATABASE_URL") else "demo","database":"ok" if os.getenv("DATABASE_URL") and db_rows("SELECT 1") else ("not_configured" if not os.getenv("DATABASE_URL") else "error"),"time":now_iso()}
@app.get("/api/funds")
def funds():
...
def nav_history(fund_code:str,points:int=Query(default=60,ge=12,le=240)):
    snap=db_rows("SELECT observed_at,estimated_nav FROM fund_estimate_snapshot WHERE fund_code=%s AND observed_at>=NOW()-INTERVAL '2 days' ORDER BY observed_at ASC LIMIT %s",(fund_code,points))
    if snap:return {"code":0,"data":[{"time":r["observed_at"].isoformat() if hasattr(r["observed_at"],"isoformat") else str(r["observed_at"]),"nav":float(r["estimated_nav"])} for r in snap],"source":"postgres-estimate-snapshots","kind":"intraday_estimate"}
...
def dca_backtest(fund_code:str|None=Query(default=None,min_length=6,max_length=6),start_nav:float|None=Query(default=None,gt=0),start_date:date|None=None,monthly_amount:float=Query(default=1000,gt=0),months:int=Query(default=36,ge=3,le=240),frequency:str=Query(default="monthly")):
    ...
    if not code and start_nav is not None:
        c=sorted(((abs(float(x["nav"])-start_nav),x["fund_code"]) for x in DEMO_FUNDS),key=lambda x:x[0]);code=c[0][1] if c and c[0][0]<0.01 else None
